...
public class DatabaseManager {
    ...
    private func open(path: String) throws {
        ...
        if path != ":memory:" {
            if sqlite3_open(path, &readDb) == SQLITE_OK {
                try exec("PRAGMA journal_mode=WAL", on: readDb)
            }
        }
    }
    ...
    private func exec(_ sql: String, on dbHandle: OpaquePointer? = nil) throws {
        let target = dbHandle ?? db
        var errMsg: UnsafeMutablePointer<CChar>?
        let rc = sqlite3_exec(target, sql, nil, nil, &errMsg)
        if rc != SQLITE_OK {
    ...
    private func read<T>(_ block: (OpaquePointer) -> T) -> T? {
        guard let rdb = readDb else { return nil }
        return block(rdb)
    }
    ...
    // MARK: - Async minutely traffic (for UI thread safety)
    public func minutelyTrafficAsync(from: Date, to: Date, completion: @escaping ([(time: Date, down: UInt64, up: UInt64, peakDown: UInt64, peakUp: UInt64, processes: String?)]) -> Void) {
        let fromStr = iso8601String(from: from)
        let toStr = iso8601String(from: to)
    ...
    // MARK: - Hourly aggregated traffic (SQL-side aggregation, ~24 rows)
    public func hourlyTraffic(from: Date, to: Date) -> [(hour: Int, down: UInt64, up: UInt64)] {
        let fromStr = iso8601String(from: from)
        let toStr = iso8601String(from: to)
        guard let rdb = readDb else { return [] }
        var stmt: OpaquePointer?
    ...
    // MARK: - Daily hourly traffic from traffic_hourly table (24 rows, instant)
    public func dailyHourlyTraffic(for date: Date) -> [(hour: Int, down: UInt64, up: UInt64)] {
        ...
        var stmt: OpaquePointer?
        let sql = """
            SELECT CAST(strftime('%H', hour) AS INTEGER) as hour,
                   total_down, total_up
            FROM traffic_hourly
            WHERE hour >= ? AND hour < ?
            ORDER BY hour
