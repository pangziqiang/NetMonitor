...
macOS 菜单栏网络监控应用，实时显示网速、CPU/GPU/内存使用率、温度、进程流量、流量历史统计（时/日/月视图）、数据导出。
...
- **时视图** — 24 小时柱状图（本地时区），实时小时缺口补齐，日期下拉选择器
- **日视图** — 最近 24 天（到今天），双行标签（周几 + MM/DD），今日桶实时更新
- **月视图** — 从当年 1 月开始 24 个月，双行标签（X月 + 年份），当月实时补齐
- **统计栏** — 总下载、总上传、峰值↓、峰值↑（含峰值发生时间）
- **共享 Y 轴最大值** — 下载/上传图表对齐，未来时段灰色 3px 细条

### 数据持久化与导出
- **SQLite** — 分钟级保留 30 天，小时级保留 730 天（支撑月视图）
...
### 进程流量钻取（v1.10.0 新增）
- **双击柱子驱进程** — 时视图/日视图的任意柱子可双击，弹出该时段（小时/天）的进程流量明细表
...
│   ├── TrafficStatsView.swift       流量统计页（时/日/月/进程钻取）
...
- 首次启动温度显示需要约 9 秒（SMC 轮询机制）；黑苹果（Hackintosh）依赖 VirtualSMC，部分传感器可能缺失或不准确
- 部分虚拟网络接口（en5-en9）默认排除以避免重复计数，可在设置中自定义前缀
- 应用未做 Apple 公证/Developer ID 签名，新设备首次打开需"右键 → 打开"放行一次；更新源需能访问 GitHub
